...
> 日期：2026-08-31  
> 状态：家长已批准并通过书面规格；实现、自动验证、候选入口部署、无 VPN iPad 第 1 题评分及 8 题整课最终 test 提交均已通过
...
## 11. 2026-08-31 整课真机验收证据

- 家长在无 VPN iPad Safari 完成 S01D39 全部 8 题评分和最终 test 提交，并明确确认整体验收通过；评分反馈约 3 秒。
- 云端结果 `e78b76f2-9c2e-4f01-a260-0f7a02f6f8fd` 为 `test/completed`、97 分、24/24 星、8/8 题，每题 1 次有效评分，`formal_completion_eligible=false`。
- 结果引用的 8 个最终私有 WAV 均存在且非空，大小为 76,502 至 131,116 字节；`sherlock-english/tmp-speaking/test/` 为空，说明本轮临时分块已清理。
- 最新 formal 结果仍早于本轮 test；没有把本次验收写入 formal、完成进度或学习档案。
