...
- 学校练习、测验和考试照片/扫描件属于正式校内学习证据，但必须保留可追溯的原始文件。
- 原始材料、逐题批改、校内错题库和学习档案只存入本地 `private/`，不得进入公开 Git。
- 03 负责保存原始证据、逐题批改和维护校内错题库；00 负责综合校内证据与线上 formal 数据并维护学习档案。
- 图片模糊、题干缺失、听力无原文/答案或答案来源不可靠时，只能登记“待确认”，不得猜测批改或形成能力结论。
- 粗心、书写和审题问题可以记录，但不得自动等同于知识未掌握。
- 单次错误默认只保留在错题库；重复出现、订正后仍错或跨场景复现后，才由 00 判断是否进入长期能力画像。

## 10. 学习档案与错题库同步

- 校内错题库记录题目级事实，03 是唯一写入者。
- 学习档案记录综合后的进度、能力变化和教学策略，00 是唯一写入者。
- 学习档案必须记录已吸收的最新校内批次或日期，防止漏收和重复吸收。
- 00 更新学习档案时必须登记日期、变更内容和证据来源；没有新证据时不得仅因时间经过而改变错题状态。
