...
import { createDeterministicProbeWav, runDirectUploadProbe } from './direct-upload-probe'

describe('direct storage upload browser probe', () => {
  it('creates an exact 150 KiB mono 16 kHz 16-bit PCM WAV without microphone input', () => {
    ...
    expect(wav.byteLength).toBe(150 * 1024)
  ...
  it('uploads one raw binary PUT, verifies server-side integrity, and reports phase timings', async () => {
    const api = {
      ...
      verifyDirectUploadProbe: vi.fn().mockResolvedValue({
        ok: true, data_kind: 'test', byte_length: 150 * 1024, sha256: 'a'.repeat(64), cleaned_up: true
      }),
    ...
    const result = await runDirectUploadProbe(api as never, 'parent-token', {
    ...
    expect(result.byte_length).toBe(150 * 1024)
  ...
  it('best-effort cancels the exact ticket after a CORS/network PUT failure', async () => {
    ...
    await expect(runDirectUploadProbe(api as never, 'parent-token', {
      fetcher: fetcher as unknown as typeof fetch
