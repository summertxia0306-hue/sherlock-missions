...
const revision = 'b83a08ddfdf0066d0d419f39c61398c6640d8bb9'
const base = `https://gcore.jsdelivr.net/gh/summertxia0306-hue/sherlock-missions@${revision}/`
const plan = JSON.parse(await readFile(
  join(root, 'content/drafts/4A-T1-W01-UNIT1-UNIT2-REVIEW/audio-generation-plan.json'),
  'utf8'
))
...
async function worker() {
  while (next < plan.items.length) {
    ...
    const manifestHash = manifests[item.module]?.courses?.[item.course_id]?.[item.path]
    assert.ok(manifestHash, `Manifest missing ${item.path}`)
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        assert.ok(online.length > 0, `Empty online audio ${item.path}`)
...
console.log(`Unit 1-2 review audio online verified: ${verified}/154 SHA-256 matches at ${revision}`)
