...
class AccessRouteTests(unittest.TestCase):
    ...
    def test_authenticated_parent_launcher_opens_test_session(self):
        ...
        selected_course = sorted(progress.all_courses())[0]
        at.button[0].click().run()

        self.assertFalse(at.exception)
        self.assertEqual(["test"], at.query_params["mode"])
        self.assertEqual([selected_course], at.query_params["course_id"])
        self.assertIn(
            "L_test_%s" % selected_course, at.session_state.filtered_state
        )
        self.assertIn(
