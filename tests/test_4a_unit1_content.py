...
DRAFT_DIR = ROOT / "content" / "drafts" / "4A-T1-W01-UNIT1"
LISTENING_DIR = ROOT / "content" / "listening"
SPEAKING_DIR = ROOT / "content" / "speaking"
DOC_DIR = ROOT / "docs" / "course-batches"
LISTENING_IDS = [f"L4A-T1-W01-D{day:02d}" for day in range(6, 11)]
SPEAKING_IDS = [f"S4A-T1-W01-D{day:02d}" for day in range(6, 11)]
PAIR_IDS = [f"4A-T1-W01-D{day:02d}" for day in range(6, 11)]
EXPECTED_COUNTS = [20, 20, 20, 25, 25]
EXPECTED_DIFFICULTIES = ["L1", "L1", "L2", "L2", "L3"]
EXPECTED_SCOPES = ["p7-p9", "p7-p10", "p7-p11", "p7-p13", "p7-p14"]
...
class Unit1ContentTests(unittest.TestCase):
    ...
    def test_d10_is_unit_review_and_earlier_courses_are_training(self):
    ...
    def test_content_stays_inside_unit1_and_excludes_unit2(self):
        allowed_markers = {
            "school building", "library", "classroom", "hall", "computer room", "sports field", "art room",
            "project", "school history", "ground floor", "bamboo", "umbrella", "wall", "three floors",
            "music room", "made of", "favourite place", "chairs", "desks", "read many interesting books",
        }
        corpus_parts = []
        for course_id in LISTENING_IDS:
    ...
    def test_study_packs_audio_plan_and_documents_cover_all_courses(self):
        ...
        mapping = (DOC_DIR / "2026-09-08-4A-Unit1-D06-D10教材与课程映射.md").read_text(encoding="utf-8")
        parent = (DOC_DIR / "2026-09-08-4A-Unit1-D06-D10家长版答案原文.md").read_text(encoding="utf-8")
