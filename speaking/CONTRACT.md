...
> 版本 v4.6 · 2026-09-23：新增 4A Unit 1–Unit 2 综合复习 S4A-T1-W01-D21–D25，题量 10/10/12/12/12，均为前 6 题 repeat、后续 QA；旧 8 题与 D16–D20 的 10/12 题继续兼容。Streamlit 继续是唯一儿童 formal 入口，CloudBase formal 保持关闭。录音仍以 `MediaRecorder` 为主链路，保留热部署旧模块自修复、正常 rerun 不重声明组件、单题组件实例稳定、WAV 双重完整性检查、私有录音引用、幂等和 formal/test 隔离；不得恢复为每次 rerun reload recorder 或以 `ScriptProcessor` 为主链路。

> 版本 v4.5 · 2026-09-17：当前唯一儿童 formal 入口为 Streamlit，CloudBase formal 保持关闭。4A Unit 3 D16–D20 经家长批准为每课 6 repeat＋4/6 QA（10/12 题）；旧课继续 6＋2（8 题）。PWA 子课程、会话续做、最终 proof 提交与 `sherlock-api` 清洗/评分/结果结构按课程题量处理，第 9–12 题保留原有三星或三次安全通过、录音私有路径及服务端 test/formal 隔离；本次只发布 Streamlit 课程，不部署 CloudBase formal。

> 版本 v4.4 · 2026-09-11：当前 Streamlit 正式入口改用浏览器原生 `MediaRecorder` 作为录音主链路，停止后解码为单声道 PCM，再统一编码为 16 kHz/16 bit WAV；`ScriptProcessor` 仅作为不支持 `MediaRecorder` 的旧浏览器回退。浏览器编码前与 Python 评分前均检查时长、静音、规律性空白帧和相邻重复帧；坏录音自动作废，不调用讯飞、不上传、不消耗有效录次。课程、评分规则、正式数据与历史记录均不变。

> 版本 v4.3 · 2026-09-01：仅 GitHub Pages 家长 `test` HTTP 入口启用私有 COS 直传优先。服务端签发 120 秒有效、绑定会话/caller/课程版本/题号/录次/字节数/SHA-256 的单对象 PUT 地址；客户端上传原始 WAV 后再由 `sherlock-api` 校验并复用原评分、proof、星级与最终私有录音链路，临时对象在成功、确定失败或取消后清理。签票不可用、对象缺失、完整性失败或过期等确定性存储错误才允许用同一录音回退 v4.2 分块；评分响应状态不明、认证、课程、音频或评分错误不得回退，避免重复评分。CloudBase 儿童 `formal` 入口、Event 整段请求及正式数据规则保持不变。
...
- GitHub Pages 家长 `test` HTTP 入口在 `VITE_SPEAKING_DIRECT_UPLOAD_TEST=true` 且服务端 `SPEAKING_DIRECT_UPLOAD_TEST_ENABLED=true` 时，以单对象私有 COS 直传为首选；上传票仅 120 秒有效，必须精确绑定会话/caller、课程版本、题号、录次、文件大小、SHA-256、MIME、take 和临时路径，响应不得包含长期 COS 密钥。服务端下载后校验 RIFF/WAVE、大小和 SHA-256，再调用既有幂等评分链路并清理临时对象。只有签票不可用、对象缺失、完整性失败或过期等确定性存储失败，才能对同一录音回退上一条分块链路；两个评分响应均状态不明时必须停止且保留原录音，不得切换分块重复评分。该功能不得用于 formal，客户端 test 诊断只显示阶段耗时、清理状态和安全错误码，不写学习记录。
...
- **Streamlit 录音组件 iOS 约束**：优先由 `MediaRecorder` 捕获浏览器原生压缩音频，
  停止后才用短生命周期 `AudioContext` 解码；每次录音使用新麦克风流并在结束后立即停轨。
  仅旧浏览器回退到 `ScriptProcessor`。静音或结构异常直接提示重录，不送评分。
...
### Streamlit MediaRecorder 录音内核（2026-09-11）

Streamlit 录音组件每次点击话筒都重新取得麦克风流，以浏览器原生 `MediaRecorder` 连续
捕获为主链路；录音停止后立即停轨，再解码、混合为单声道并统一编码为 16 kHz/16 bit
PCM/WAV。`ScriptProcessor` 仅在浏览器没有 `MediaRecorder` 时启用，不再作为 iPad 主链路。

浏览器提交前和 Python 调用讯飞前各做一次独立完整性检查。以下情况本次录音自动作废并
恢复可点击话筒：格式不符、少于 0.4 秒、峰值低于 0.01、约 85/93 ms 空白帧规律交替，
或同长度相邻音频帧成对重复。作废录音不上传、不评分、不消耗有效录次；普通句首静音、
自然停顿和连续正常语音不应命中。
