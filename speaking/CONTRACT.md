...
### Streamlit iOS 录音隔离（2026-09-09）

Streamlit 录音组件不再跨录次复用 `AudioContext`。每次点击话筒时在用户手势栈中创建
新的上下文并重新获取麦克风流；完成、失败、放弃、切题或页面离开时停止音轨并关闭当次
上下文。

编码前检查 4096 采样缓冲块。若大量全零块与有声块呈严格交替，按采集断续处理：本次
录音作废、恢复可点击话筒，不上传、不调用讯飞、不消耗评分录次。普通句首静音、停顿及
完整静音仍分别由既有时长/峰值规则处理。
